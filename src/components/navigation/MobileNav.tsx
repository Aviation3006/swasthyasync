...
export const MobileNav: React.FC = () => {
  ...
  return (
    <nav 
      ...
      className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200 px-1 py-1 shadow-elevated pb-safe"
    >
        ...
        {links.map((link) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex flex-col items-center justify-center py-1 px-1 min-h-[48px] rounded-md text-[10px] font-semibold transition-colors select-none ${
                  isActive
                    ? 'text-theme-primary font-extrabold bg-theme-primary-light shadow-2xs border-b-2 border-theme-primary'
                    : 'text-slate-600 hover:text-slate-900 active:bg-slate-100'
                }`
              }
            >
              <Icon className="w-5 h-5 mb-0.5 shrink-0" />
              <span className="text-[10px] leading-tight text-center truncate max-w-full px-0.5">{link.label}</span>
            </NavLink>
          );
        })}
      </div>
